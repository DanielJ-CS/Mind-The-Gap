...
public class ArrivalsListFragment extends ListFragment {
    ...
    public ArrayList<Arrival> getArrivalsForSelectedStationOnLineInDirection(String lineId, String travelDirn) {
        ...
        for (Iterator itr = s.iterator();itr.hasNext();) {
            ArrivalBoard ab = (ArrivalBoard) itr.next();
            if ((ab.getLine().getId().equals(lineId)) && (travelDirn.equals(ab.getTravelDirn()))) {
                for (Iterator itr2 = ab.iterator();itr2.hasNext();) {
                    Arrival a = (Arrival) itr2.next();
                    arrivals.add(a);
                }
                }
        }
